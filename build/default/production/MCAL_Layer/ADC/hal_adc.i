...
static __attribute__((inline)) void configure_adc_volatage_ref(const adc_t *adc_obj);
static __attribute__((inline)) void configure_adc_result_format(const adc_t *adc_obj);
static __attribute__((inline)) void configure_adc_channel_port(adc_channel_t adc_ch);
...
Std_RetrunType ADC_get_conversion_result(const adc_t *adc_obj,uint16 *adc_result){
    ...
    {
        if(adc_obj->result_format == 0U)
        {
            *adc_result=(uint16)(ADRESH+((ADRESL>>6)<<8));

        }
        else if(adc_obj->result_format == 1U)
        {
...
static __attribute__((inline)) void configure_adc_volatage_ref(const adc_t *adc_obj){
    if(adc_obj->voltage_cfg == 1U){

        do{ADCON1bits.VCFG1=1; ADCON1bits.VCFG0=1; }while(0);;
    }
    else if(adc_obj->voltage_cfg == 0U){
